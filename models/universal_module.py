...
class MACHActivationHebbian(nn.Module):
    ...
    def compute_context_gates(self):
        """PFC: integrate all-layer activations through recurrent circuit → context gates.
        Like PFC reading early (concrete) + late (abstract) cortical areas,
        ...
        # Hippocampal reinstatement adds to _pfc_state every retrieval, bypassing
        # GRU's tanh bound. Without clamping, state drifted to 10^17 at 98k steps.
        # Per-element clamp (not unit-norm): preserves direction + magnitude info
        # that context_gates need for task differentiation. GRU tanh gives [-1,1]
        ...
        for i in range(self.n_patches):
            ...
            if pre is not None:
                ...
                summaries.append(torch.zeros(
                    ...
                    device=self._critic_state.device,
                ))
