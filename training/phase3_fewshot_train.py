"""
Ablation: Phase 3 architecture + few-shot task.
No cerebellum, no surprise, no TD modulation, no obs pathway, no recency.
Tests whether the task structure alone drives few-shot learning.
"""
...
def run_episode_phase3_fewshot(base_model, mach, patched_model, tokenizer,
                                problems, device, gamma=None):
    """
    Phase 3 episode with few-shot demo handling.
    Plain CE loss — no TD modulation, no recency weighting.
    """
    if gamma is None:
...
def meta_train_phase3_fewshot(base_model, mach, patched_model, tokenizer,
                               device, n_episodes=None, lr=None,
                               curriculum=None, checkpoint_path=None):
    """Phase 3 ablation: few-shot task with Phase 3 architecture."""
    ...
    print(f"Phase 3 (fewshot ablation) trainable parameters: {n_meta:,}")
    print(f"Phase 3 gamma: {config.PHASE3_GAMMA}")
    ...
    for episode_idx in range(n_episodes):
        ...
        try:
            loss, rewards, problem_losses, diagnostics = \
                run_episode_phase3_fewshot(
...
def _run_standard_validation(base_model, mach, patched_model, tokenizer,
    ...
    print(
        ...
        f"early={early_acc:.0%} late={late_acc:.0%} "
        f"delta={delta:+.0%} avg_v={mean_val:.3f}"
    )
