...
DEFAULT_CURRICULUM = [
    (0, 200, "single"),      # Short warm-up (loading Phase 3 checkpoint)
    (200, 2000, "mixed"),    # Mixed d5/d6/d7
]
...
def run_episode_phase6(base_model, mach, patched_model, tokenizer,
                       problems, device, gamma=None):
    """
    Phase 6 episode with planning loop.

    Key differences from Phase 3:
    - fire() runs multiple transformer iterations internally
    - Critic loss computed for ALL iterations per problem (not just committed)
    - Tracks commit distribution for diagnostics
    """
    if gamma is None:
    ...
    all_iteration_values = []  # list of lists, for multi-iter critic loss
    ...
    for i, problem in enumerate(problems):
        ...
        # Critic loss for ALL iterations (not just committed)
        if i > 0:
            td_target = last_reward + gamma * current_value.detach()
            td_error = td_target - values[i - 1]
            last_td_error = td_error.detach()

            # Standard critic loss for committed value of previous step
            critic_loss = critic_loss + (values[i - 1] - td_target.detach()) ** 2

            # Multi-iteration critic loss: all iterations of previous step
            # should also predict value accurately
            prev_iter_values = all_iteration_values[i - 1]
            for iter_val in prev_iter_values:
                if iter_val is not values[i - 1]:  # skip committed (already counted)
                    critic_loss = critic_loss + (iter_val - td_target.detach()) ** 2
    ...
    if len(values) > 0:
        ...
        # All iteration values for last step
        last_iter_values = all_iteration_values[-1]
        for iter_val in last_iter_values:
            if iter_val is not values[-1]:
                critic_loss = critic_loss + (iter_val - td_target_final.detach()) ** 2
    ...
    # Commit distribution as fractions
    for k in range(mach.max_planning_iters):
        diagnostics[f"commit_iter_{k}"] = (
            commit_counts[k] / n_problems_total if n_problems_total > 0 else 0
        )

    return total_loss, rewards, problem_losses, diagnostics


def meta_train_phase6(base_model, mach, patched_model, tokenizer, device,
                      n_episodes=None, lr=None, curriculum=None,
                      checkpoint_path=None):
    """
    Phase 6 meta-training loop.

    Same parameter set as Phase 3 (zero new parameters).
    Planning loop is purely computational graph topology.
    """
    if n_episodes is None:
        n_episodes = config.PHASE6_EPISODES
    if lr is None:
        lr = config.PHASE6_LR
    ...
    print(f"Phase 6 meta-learner trainable parameters: {n_meta_params:,}")
    ...
    def get_n_problems(episode_idx):
        if episode_idx < 100:
            ...
            return config.PHASE6_PROBLEMS_PER_EPISODE
    ...
    for episode_idx in range(n_episodes):
        ...
        try:
            loss, rewards, problem_losses, diagnostics = run_episode_phase6(
        ...
        torch.nn.utils.clip_grad_norm_(
            meta_params, max_norm=config.PHASE6_GRAD_CLIP
        )
        optimizer.step()
        ...
        if episode_idx % 10 == 0:
            ...
            # Commit distribution string
            commit_str = " ".join(
                f"i{k}={diagnostics[f'commit_iter_{k}']:.0%}"
                for k in range(mach.max_planning_iters)
            )
            ...
            for k in range(mach.max_planning_iters):
                log_dict[f"commit_iter_{k}"] = diagnostics[f"commit_iter_{k}"]
        ...
        if (episode_idx % 200 == 0 and episode_idx > 0) or episode_idx == n_episodes - 1:
            _log_diagnostics_phase6(mach, meta_params, episode_idx)
            for eval_diff in [5, 6, 7]:
                _run_validation_phase6(
                    base_model, mach, patched_model, tokenizer,
                    device, eval_diff, episode_idx
                )


def _run_validation_phase6(base_model, mach, patched_model, tokenizer, device,
                           difficulty, episode_idx, n_episodes=10, n_problems=20):
    """
    Held-out evaluation with planning loop diagnostics.
    Reports commit distribution and whether committed iterations have
    higher critic values than non-committed.
    """
    base_correct = 0
    ...
    all_values = []
    commit_counts = [0] * mach.max_planning_iters
    committed_vs_others = []  # (committed_value, mean_other_value)
    total_problems = 0
    ...
    for ep in range(n_episodes):
        ...
        for i, problem in enumerate(problems):
            ...
            # Track committed vs non-committed values
    ...
    if committed_vs_others:
        ...
        critic_str = f"committed_v={avg_committed:.3f} other_v={avg_other:.3f} {'OK' if critic_correct else 'BAD'}"
    else:
        critic_str = "n/a"
    ...
    print(f"  EVAL ep{episode_idx} d={difficulty} | "
          ...
          f"commit: {commit_str} | {critic_str}")
    ...
    if wandb is not None:
        wandb.log({
            ...
            **{f"eval/d{difficulty}_commit_iter_{k}": commit_dist[k]
               for k in range(mach.max_planning_iters)},
        })


def _log_diagnostics_phase6(mach, meta_params, episode_idx):
